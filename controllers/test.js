...
*/
//New code To createTest by abir
export async function createTest(req, res) {
  try {
    const { idTeacher, title, description, duration, questions, testDate } = req.body;
    ...
    questionObjects.forEach(question => {
      if (!questionsByChapter[question.chapitre]) {
        questionsByChapter[question.chapitre] = [];
      }
      questionsByChapter[question.chapitre].push(question);
    });

    // Calculate total marks per chapter
    const marksByChapter = {};
    Object.keys(questionsByChapter).forEach(chapitre => {
      const questionsInChapter = questionsByChapter[chapitre];
      const totalMarks = questionsInChapter.reduce((total, question) => total + question.marks, 0);
      marksByChapter[chapitre] = totalMarks;
    });

    // Convert marksByChapter to string
    const marksByChapterString = JSON.stringify(marksByChapter);
    ...
    const newTest = new Test({
      ...
      questions: questionObjects,
      testDate,
      marksByChapter: marksByChapterString // Assigning the stringified marksByChapter
    });
